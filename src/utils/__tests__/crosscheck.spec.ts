...
import { departureAt, detectDayType, toTimeString, tripsFor } from '@/utils/timetable'
...
describe('tripsFor', () => {
  it('narrows the poster to the population the MC query asks for', () => {
    ...
    expect(tripsFor('weekday', 'inbound', ['MC', 'PE'])).toHaveLength(123)
  })
  ...
  it('excludes trains that do not call at the station', () => {
    ...
    const narrowed = tripsFor('weekday', 'inbound', ['MC', 'PE'])
  ...
  it('rejects an unknown station rather than silently matching nothing', () => {
    expect(() => tripsFor('weekday', 'inbound', ['XX'])).toThrow(/XX/)
  })
})

describe('crossCheckSchedule', () => {
  const trips = tripsFor('weekday', 'inbound', ['MC', 'PE'])
  ...
  it('keeps trains past midnight after the evening ones', () => {
    ...
    const outbound = tripsFor('weekday', 'outbound', ['PE', 'MC'])
...
describe('detectDayType', () => {
  const timesAt = (dayType: Parameters<typeof tripsFor>[0]) =>
    tripsFor(dayType, 'inbound', ['MC', 'PE']).map((trip) => departureAt(trip, 'MC')!)

  it('picks the pattern the reported times actually match', () => {
    const saturday = new Date(2026, 8, 19)

    expect(detectDayType(timesAt('weekday'), 'MC', 'inbound', saturday)).toBe('weekday')
    expect(detectDayType(timesAt('saturdayHoliday'), 'MC', 'inbound', saturday)).toBe(
      'saturdayHoliday'
    )
  })

  it('falls back to the calendar when there is nothing to score', () => {
    expect(detectDayType([], 'MC', 'inbound', new Date(2026, 8, 16))).toBe('weekday')
    expect(detectDayType([], 'MC', 'inbound', new Date(2026, 8, 19))).toBe('saturdayHoliday')
    expect(detectDayType([], 'MC', 'inbound', new Date(2026, 7, 12))).toBe('augustWeekday')
  })
})
