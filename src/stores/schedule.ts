    ...
    async fetchScheduleMC() {
      try {
        const data = await axios.get(
          'https://fgc.opendatasoft.com/api/explore/v2.1/catalog/datasets/viajes-de-hoy/records?limit=100&refine=trip_headsign%3APl.%20Espanya&refine=stop_id%3A%22MC%22'
        )
        this.scheduleMC = data.data.results

        this.scheduleMCTimeFiltered = this.scheduleMC
          .map((x) => {
